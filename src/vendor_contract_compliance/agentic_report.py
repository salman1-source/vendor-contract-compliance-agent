...
def write_agentic_report(result: AgenticRunResult, output: Path):
    ...
    lines=["# Phase 3A Agentic Audit","",f"> {DISCLAIMER}","",f"- Model: `{result.model_name}`",f"- Client: `{result.client_type}`",f"- Route: `{' -> '.join(result.graph_path)}`",f"- Retries: {result.retry_count}",f"- Reviewer: **{result.reviewer_decision}**","","## Findings","","| Policy | Status | Severity | Page |","|---|---|---|---|"]
